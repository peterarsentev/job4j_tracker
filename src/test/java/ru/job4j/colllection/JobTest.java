...
public class JobTest {
    ...
    public void whenCompatorByNameAndPrority() {
        Comparator<Job> cmpNamePriority = new JobDescByNameDecr().thenComparing(new JobDescByPriorityDecr());
    ...
    public void whenCompatorByNameIncr() {
        ...
        int rsl = compatorByNameIncr.compare(new Job("Impl task", 0),new Job("Fix bug", 1));
        assertThat(rsl,greaterThan(0));
    }
    @Test
    public void whenCompatorByNameDecr() {
        Comparator<Job> compatorByNameDecr = new JobDescByNameDecr();
        int rsl = compatorByNameDecr.compare(new Job("Impl task", 0),new Job("Fix bug", 1));
        assertThat(rsl,lessThan(0));    }
    ...
    public void whenCompatorByNameAndProrityInc() {
        Comparator<Job> cmpNamePriority = new JobDescByNameIncr().thenComparing(new JobDescByPriorityInc());
        int rsl = cmpNamePriority.compare(
                new Job("Impl task", 0),
                new Job("Fix bug", 1)
        );
